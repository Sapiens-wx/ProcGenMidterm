...
public class ValidatedRoom : Room
{
    ...
    /*SearchVertex GetPath(Vector2Int start, Vector2Int target)
    {
        List<SearchVertex> openSet = new();
        List<Vector2Int> closedSet = new();
        
        SearchVertex startVertex = new SearchVertex();
        startVertex.gridPos = start;
    ...
    {
        loadData();
        Vector2Int topExit = new Vector2Int(0, 4);
    ...
    bool HasPath(Vector2Int start, Vector2Int target)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            if (CheckNeighbor(upNeighbor, closedSet, openSet))
                openSet.Add(upNeighbor);
            if (CheckNeighbor(downNeighbor, closedSet, openSet))
                openSet.Add(downNeighbor);
            if (CheckNeighbor(leftNeighbor, closedSet, openSet))
                openSet.Add(leftNeighbor);
            if (CheckNeighbor(rightNeighbor, closedSet,  openSet))
    ...
    bool CheckNeighbor(Vector2Int node, List<Vector2Int> closedSet, List<Vector2Int> openSet)
    {
        return IsInBounds(node) 
               && !closedSet.Contains(node) 
               && !openSet.Contains(node)
               && _indexGrid[node.x, node.y] == 0;
    }
